...
export class WeatherService {
  ...
  static async getCurrentWeather(latitude: number, longitude: number): Promise<WeatherData> {
    if (!OPENWEATHER_API_KEY) {
      throw new Error('OpenWeatherMap API 密钥未配置！请在 .env 文件中设置 OPENWEATHER_API_KEY');
    }

    try {
      const response = await axios.get(
        `${this.API_BASE_URL}/weather?lat=${latitude}&lon=${longitude}&appid=${OPENWEATHER_API_KEY}&units=metric&lang=zh_cn`
      );
